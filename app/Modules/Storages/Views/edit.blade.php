...
@section('title', $data->data->id.' - '.$data->data->shop_name)
@section('otherhead')
...
@section('content')
    <div class="mainPAge">
        <div class="row first-row" >
            <form method="post" action="{{ URL::to('/storages/update/'.$data->data->shop_id) }}" class="form-horizontal form-label-left" enctype="multipart/form-data">
                <div class="col-md-12 col-sm-12 col-xs-12" >
                    <div class="x_panel" >
                        <div class="x_title">
                            <strong>تعديل بيانات الصندوق</strong>
                            <ul class="nav navbar-right panel_toolbox">
                                <div align="right">
                                    ...
                                    <button type="submit" class="btn btn-round btn-success addSupply" data-area="{{ $data->data->shop_id }}">تعديل <i class="fa fa-plus"></i></button>
                                    @endif
                                </div>
                            </ul>
                            ...
                            <div class="row first-supply">
                                <div class="col-md-4 col-xs-12 col-sm-6">
                                    <div class="form-group">
                                        <label>الفرع</label>
                                        <select class="form-control shop_id select2" disabled>
                    ...
                    <div class="x_content x_content_playlist">
                        <div class="col-xs-12 col-sm-12 col-md-12 first-content">
                            <div class="col-xs-12 col-md-6">
                                ...
                                <input type="hidden" name="items" value="{{ json_encode($data->data->myData) }}">
                                <div class="clearfix"></div>
                            </div>
                            <div class="col-xs-12 col-md-6 data-table">
                                <table class="table hover first-table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @foreach($data->data->currency_id as $key => $one)
                                        <tr>
                                            <td>{{ $data->data->shop_name }}</td>
                                            <td>{{ $data->data->balance[$key] }}</td>
                                            <td>{{ $data->data->currency_name[$key] }}</td>
                                            <td>
                                                <button class="btn btn-xs btn-danger removeRecord" data-area="{{ $data->data->currency_id[$key] }}"><i class="fa fa-trash"></i> حذف</button>
                                            </td>
                                        </tr>
                                        @endforeach 
                                    </tbody>
...
@stop()
@section('script')
    <script src="{{ asset('assets/components/edit-storage.js')}}"></script>
@stop()
